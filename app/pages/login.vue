<template>
  <div class="min-h-[calc(100dvh-61px)] flex bg-surface overflow-hidden">
    <Head>
      <title>{{ t("auth.signIn") }} - VoyageHub</title>
      <meta :content="t('auth.loginSubtitle')" name="description" />
    </Head>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-8 lg:p-10 relative">
      ...
      <div class="w-full max-w-md space-y-5 lg:space-y-4">
        <div>
          <h2 class="text-3xl font-bold text-on-surface mb-2 tracking-tight">{{ t("auth.loginTitle") }}</h2>
          <p class="text-on-surface-variant text-sm">{{ t("auth.loginSubtitle") }}</p>
        </div>
        ...
        <div class="flex items-center gap-4 py-2">
          ...
          <span class="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">{{ t("auth.loginOrEmail") }}</span>
          <div class="flex-1 h-px bg-outline-variant/30"></div>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="space-y-4 lg:space-y-3">
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <input
                ...
                autocomplete="email"
                required
                class="w-full pl-10 pr-4 py-2 bg-surface-container-lowest border border-outline-variant/50 rounded-xl text-sm text-on-surface placeholder:text-outline/60 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none"
              />
            </div>
          </div>
          ...
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              ...
              <NuxtLink to="/forgot-password" class="text-sm font-medium text-primary hover:text-primary-container transition-colors">{{ t("auth.forgotPassword") }}</NuxtLink>
            </div>
            <div class="relative">
              ...
              <input
                ...
                required
                class="w-full pl-10 pr-10 py-2 bg-surface-container-lowest border border-outline-variant/50 rounded-xl text-sm text-on-surface placeholder:text-outline/60 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none"
              />
          ...
          <button type="submit" :disabled="loading" class="w-full flex items-center justify-center gap-2 py-2.5 px-4 mt-2 bg-gradient-to-br from-primary to-primary-container text-white font-bold rounded-xl hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/25 transition-all disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none">
            ...
            <span v-else class="text-sm">{{ t("auth.signIn") }}</span>
          </button>
        </form>

        <p class="text-center text-sm text-on-surface-variant pt-2">
          {{ t("auth.noAccount") }}
          <NuxtLink to="/register" class="font-bold text-primary hover:text-primary-container hover:underline underline-offset-4 transition-all">{{ t("auth.createOneFree") }}</NuxtLink>
        </p>
      </div>
    </div>
  </div>
