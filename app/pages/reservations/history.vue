    ...
    <div class="profile-container">
      <header class="page-header">
        <div class="page-header__content">
          ...
          <p class="page-header__sub">Explore your journey history and manage your upcoming stays.</p>
        </div>
        ...
        <main class="profile-main">
          ...
          <div class="control-panel">
            ...
            <div class="filters-wrap">
              <div class="status-pills">
                ...
                >
                  <span class="material-symbols-outlined pill-icon" v-if="s.value">{{ statusIcon(s.value) }}</span>
                  {{ s.label }}
                </button>
              </div>
          ...
          <div v-else-if="filtered.length === 0" class="empty-display">
            <div class="empty-card">
              ...
              <p class="empty-text">We couldn't find any bookings matching your current filters. Start exploring to plan your next adventure!</p>
              <div class="empty-actions">
                <NuxtLink to="/hotels" class="btn-primary">
                  <span class="material-symbols-outlined">explore</span>
                  Explore Destinations
                </NuxtLink>
                <button v-if="search || statusFilter || yearFilter" @click="resetFilters" class="btn-ghost">
                  Clear All Filters
                </button>
              </div>
            </div>
          ...
          <div v-else class="history-timeline">
            <template v-for="(group, year) in grouped" :key="year">
              ...
              <div class="reservation-grid">
                <div v-for="r in group" :key="r.id" class="journey-card" :class="`journey-card--${r.status.toLowerCase()}`">
                  <div class="journey-card__image">
                    <img v-if="getHotel(r.hotelId)" :src="getHotel(r.hotelId)?.images[0]" :alt="getHotel(r.hotelId)?.name" />
                    <div v-else class="image-placeholder">
                      <span class="material-symbols-outlined">hotel</span>
                    </div>
                    <div class="journey-status-tag" :class="badgeClass(r.status)">
                  ...
                  <div class="journey-card__content">
                    <div class="journey-card__header">
                      <div class="journey-card__id">REF: {{ r.confirmationCode }}</div>
                      <h3 class="journey-card__title">{{ getHotel(r.hotelId)?.name || 'Hotel #' + r.hotelId }}</h3>
                    ...
                    <div class="journey-card__details">
                      <div class="detail-block">
                        ...
                        <span class="detail-value">{{ formatDate(r.checkInDate) }}</span>
                      </div>
                      <div class="detail-block">
                        <span class="detail-label">Stay Duration</span>
                        <span class="detail-value">{{ r.numberOfNights }} Night{{ r.numberOfNights > 1 ? 's' : '' }}</span>
                      </div>
                      <div class="detail-block detail-block--price">
                        <span class="detail-label">Total Cost</span>
                        <span class="detail-value">{{ formatCurrency(r.totalAmount) }}</span>
                      </div>
                    </div>

                    <div class="journey-card__footer">
                      <NuxtLink :to="`/hotels/${r.hotelId}`" class="btn-card-action">
                        View Details
...
.badge--confirmed { background: rgba(0, 103, 104, 0.9); color: #fff; }
.badge--completed { background: rgba(16, 185, 129, 0.9); color: #fff; }
.badge--cancelled { background: rgba(239, 68, 68, 0.9); color: #fff; }
.badge--pending   { background: rgba(245, 158, 11, 0.9); color: #fff; }
...
@media (max-width: 900px) {
  ...
  .journey-card__image { width: 100%; height: 180px; }
}

@media (max-width: 600px) {
  .journey-card__details { grid-template-columns: 1fr; gap: 16px; }
  .control-panel { padding: 16px; }
  .status-pills { overflow-x: auto; padding-bottom: 8px; }
}
</style>
