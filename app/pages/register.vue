<template>
  <div class="min-h-[calc(100dvh-61px)] flex bg-surface overflow-hidden">
    <Head>
      <title>{{ t("auth.createAccount") }} - VoyageHub</title>
      <meta :content="t('auth.createAccountSubtitle')" name="description" />
    </Head>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-8 lg:p-10 relative">
      ...
      <div class="w-full max-w-md space-y-5 lg:space-y-4">
        <div>
          <h2 class="text-3xl font-bold text-on-surface mb-2 tracking-tight">{{ t("auth.createAccountTitle") }}</h2>
          <p class="text-on-surface-variant text-sm">{{ t("auth.createAccountSubtitle") }}</p>
        </div>

        <!-- Google Sign In -->
        <button type="button" @click="handleGoogleSignIn" class="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-surface-container-lowest border border-outline-variant/50 rounded-xl text-on-surface font-semibold hover:bg-surface-container-low transition-colors shadow-sm">
        ...
        <div class="flex items-center gap-4 py-2">
          ...
          <span class="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">{{ t("auth.registerOrEmail") }}</span>
          <div class="flex-1 h-px bg-outline-variant/30"></div>
        </div>
        ...
        <form @submit.prevent="handleRegister" novalidate class="space-y-3 lg:space-y-2.5">
          ...
          <div class="grid grid-cols-2 gap-4 lg:gap-3">
            <div class="space-y-2">
              <label class="block text-sm font-semibold text-on-surface" for="firstName">{{ t("auth.firstName") }} <span class="text-error">*</span></label>
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full pl-10 pr-4 py-2 text-sm bg-surface-container-lowest border rounded-xl text-on-surface placeholder:text-outline/60 focus:ring-2 focus:ring-primary/20 transition-all outline-none"
                  :class="submitted && !firstName ? 'border-error focus:border-error' : 'border-outline-variant/50 focus:border-primary'"
                />
              </div>
              <p v-if="submitted && !firstName" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.firstNameRequired") }}</p>
            </div>
            <div class="space-y-2">
              <label class="block text-sm font-semibold text-on-surface" for="lastName">{{ t("auth.lastName") }}</label>
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full pl-10 pr-4 py-2 text-sm bg-surface-container-lowest border border-outline-variant/50 rounded-xl text-on-surface placeholder:text-outline/60 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none"
                />
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2">
            <label class="block text-sm font-semibold text-on-surface" for="email">{{ t("auth.emailAddress") }} <span class="text-error">*</span></label>
            <div class="relative">
              ...
              <input
                ...
                autocomplete="email"
                class="w-full pl-11 pr-4 py-2 text-sm bg-surface-container-lowest border rounded-xl text-on-surface placeholder:text-outline/60 focus:ring-2 focus:ring-primary/20 transition-all outline-none"
                :class="submitted && (!email || !isValidEmail) ? 'border-error focus:border-error' : 'border-outline-variant/50 focus:border-primary'"
              />
            </div>
            <p v-if="submitted && !email" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.emailRequired") }}</p>
            <p v-else-if="submitted && !isValidEmail" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.emailInvalid") }}</p>
          </div>

          <div class="space-y-2">
            <label class="block text-sm font-semibold text-on-surface" for="password">{{ t("auth.password") }} <span class="text-error">*</span></label>
            <div class="relative">
              ...
              <input
                ...
                class="w-full pl-11 pr-12 py-2 text-sm bg-surface-container-lowest border rounded-xl text-on-surface placeholder:text-outline/60 focus:ring-2 focus:ring-primary/20 transition-all outline-none"
            ...
            <p v-if="submitted && !password" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.passwordRequired") }}</p>
            <p v-else-if="submitted && !isPasswordValid" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.passwordInvalid") }}</p>
          </div>

          <div class="space-y-2">
            <label class="block text-sm font-semibold text-on-surface" for="confirmPassword">{{ t("auth.confirmPassword") }} <span class="text-error">*</span></label>
            <div class="relative">
            ...
            <p v-if="submitted && !confirmPassword" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.confirmPasswordRequired") }}</p>
            <p v-else-if="submitted && passwordMismatch" class="text-xs text-error font-medium flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.passwordsMismatch") }}</p>
          </div>
          ...
          <div>
            <label class="flex items-start gap-3 cursor-pointer pt-1">
              ...
              <span class="text-sm text-on-surface-variant leading-snug">
                {{ t("auth.agreePrefix") }}
                <NuxtLink to="/terms" class="font-semibold text-primary hover:underline">{{ t("auth.terms") }}</NuxtLink>
                {{ " " }}
                <span>{{ t("auth.and") }}</span>
                {{ " " }}
                <NuxtLink to="/privacy" class="font-semibold text-primary hover:underline">{{ t("auth.privacy") }}</NuxtLink>
                <span class="text-error">*</span>
              </span>
            </label>
            <p v-if="submitted && !agreed" class="text-xs text-error font-medium mt-1 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">error</span> {{ t("auth.agreeRequired") }}</p>
          </div>
          ...
          <button type="submit" :disabled="loading" class="w-full flex items-center justify-center gap-2 py-2.5 bg-gradient-to-br from-primary to-primary-container text-white font-bold rounded-xl hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/25 transition-all disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none mt-1">
            ...
            <span v-else class="text-sm">{{ t("auth.createAccount") }}</span>
          </button>
        </form>

        <p class="text-center text-sm text-on-surface-variant pt-1">
          {{ t("auth.alreadyHaveAccount") }}
          <NuxtLink to="/login" class="font-bold text-primary hover:text-primary-container hover:underline underline-offset-4 transition-all">{{ t("auth.signIn") }}</NuxtLink>
        </p>
      </div>
    </div>
  </div>
...
async function handleGoogleSignIn() {
  ...
  try {
    ...
    error.value = err.message || t('auth.googleFailed')
  } finally {
    loading.value = false
  }
}
